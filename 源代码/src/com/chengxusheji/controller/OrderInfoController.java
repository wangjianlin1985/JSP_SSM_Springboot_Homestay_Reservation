...
public class OrderInfoController extends BaseController {
	...
	public void ownerList(@ModelAttribute("userObj") UserInfo userObj,String liveDate,String orderState,String orderTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response,HttpSession session) throws Exception {
		if (page==null || page == 0) page = 1;
		if (liveDate == null) liveDate = "";
		if (orderState == null) orderState = "";
		if (orderTime == null) orderTime = "";
		...
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(owner, userObj, liveDate, orderState, orderTime, page);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(owner, userObj, liveDate, orderState, orderTime);
	    /*获取到总的页码数目*/
	    int totalPage = orderInfoService.getTotalPage();
	    /*当前查询条件下总记录数*/
	    int recordNumber = orderInfoService.getRecordNumber();
	...
	public String userFrontlist(@ModelAttribute("minsuObj") Minsu minsuObj,@ModelAttribute("userObj") UserInfo userObj,String liveDate,String orderState,String orderTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (liveDate == null) liveDate = "";
		if (orderState == null) orderState = "";
		if (orderTime == null) orderTime = "";
